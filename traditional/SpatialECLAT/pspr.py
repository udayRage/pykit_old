...
class Tree(object):
    ...
    def genrate_tids(self,node):
        ...
        return final_tids
    def remove_node(self,node_val,gene_list):
    ...
    def generate_patterns(self,prefix,nbh):
        for i in sorted(self.summaries,reverse = True):
        	...
        	# print(gene_list[i],pf)
        	if(pf>=per_freq):
        		pattern=prefix.copy()
        		pattern.append(gene_list[i])
        		if (pf/(supp_dict[pattern[0]]-1))>=min_rps:
        			yield (pattern,pf)        		
...
def main():    
    ...
    # periodicity=(periodicity*total_transactions)/100
