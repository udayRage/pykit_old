...
class Tree(object):
    ...
    def get_condition_pattern(self,alpha,new_nbh):
        ...
        final_patterns,final_sets=cond_trans(final_patterns,final_sets)
...
def cond_trans(cond_pat,cond_tids):
    ...
    up_dict={}
    for m in data1:
        up_dict[m]=[verify_tids(data1[m]),len(data1[m])]
    up_dict={k: v for k,v in up_dict.items() if v[0]<per_freq}
    count=0
    for p in cond_pat:
        trans=[]
        for q in p:
