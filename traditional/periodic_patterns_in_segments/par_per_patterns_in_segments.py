...
class Node(object):
    ...
    def size_of_node(self):
        size=getsizeof(self.item)+getsizeof(self.children)+getsizeof(self.parent)+getsizeof(self.summaries)+getsizeof(self.freq)
        return size
...
class Tree(object):
    ...
    def remove_node(self,node_val):
        for i in self.summaries[node_val]:
            ...
            i=None
    def get_ts(self,alpha,per,x_fac,max_non_irregul,min_pf,min_sup):
        tid_s=set()
        freq=0
        per_fre=0
        valid=0
        for i in self.summaries[alpha]:
            ...
            freq+=i.freq
        if freq>min_sup:
            per_fre,valid=get_per_fre(tid_s,per,x_fac,max_non_irregul)
            if(valid!=0):
                if(per_fre<min_pf):
                    valid=0
        return per_fre,freq,valid        
    def generate_patterns(self,prefix,per,min_pf,min_sup,x_fac,max_non_irregul,genelist):
        for i in sorted(self.summaries,reverse = True): 
#             print(genelist[i])
            per_fre,freq,valid=self.get_ts(i,per,x_fac,max_non_irregul,min_pf,min_sup)
            if(valid==1):
                pattern=prefix.copy()
                ...
                if(len(patterns)>=1):
                    for li in conditional_tree.generate_patterns(pattern,per,min_pf,min_sup,x_fac,max_non_irregul,genelist):
                        yield (li)
            self.remove_node(i)
...
def get_per_fre(tids,per,x_fac,max_non_irregul):
    tids = list(tids)
    tids.sort()
    cur=tids[0]
    irregu=0
    ...
    for j in range(1,len(tids)):
        if(tids[j]-cur<=per):
            ...
            if(tids[j]-cur>x_fac*per):
                irregu+=1
                if(irregu>max_non_irregul):
                    valid=0
                    break
        cur=tids[j]
    return pf,valid
...
def generate_dict(transactions,per_freq,min_sup,periodicity,x_fac,max_non_irregul):
    data={}
    max_acc_per=x_fac*periodicity
    for tr in transactions:
        for i in range(1,len(tr)):
            if tr[i] not in data:
                data[tr[i]]=[int(tr[0]),1,0,0]
            else:
                if((int(tr[0])-data[tr[i]][0]) <= periodicity):
                    data[tr[i]][2]+=1
                else:
                    if((int(tr[0])-data[tr[i]][0]) > max_acc_per):
                        data[tr[i]][3]+=1
                data[tr[i]][0]=int(tr[0])
                data[tr[i]][1]+=1
#     print(data)
    data={k: v for k,v in data.items() if v[2]>=per_freq and v[1]>=min_sup and v[3]<=max_non_irregul}
    return data
...
def main(path,per_freq,periodicity,min_sup,length,x_fac,max_non_irregul):    
    ...
    total_transactions=len(list_of_segments)
#     print(list_of_segments)
    ...
    max_non_irregul=(max_non_irregul*total_transactions)/100
    print(per_freq,per,min_sup)
    generated_dict=generate_dict(list_of_segments,per_freq,min_sup,per,x_fac,max_non_irregul)
...
#     rank = dict([(index,item) for (item,index) in enumerate(gene_list)])
    ...
    q=Tree.generate_patterns([],per,per_freq,min_sup,x_fac,max_non_irregul,gene_list)
    return q,gene_list
...
if(__name__ == "__main__"):
    ...
    x_fac=float(sys.argv[7])
    max_non_irregul=float(sys.argv[8])
    k,gl=main(path,per_freq,periodicity_threshold,min_sup,length,x_fac,max_non_irregul)
